...
$.confirm.defaults = {
	text: {
		header: 'Confirmez votre action svp...',
		ok: 'Oui',
		cancel: 'Non'
	}
};
...
$.msg = function (content, options) {
	...
	function getHTML(content, headline) {
		return '<div class="msg-box"><h3>' + (headline || '') + '</h3><a class="msg-close">Fermer</a><div class="msg-content">' + content.replace('\n', '<br>') + '</div></div>';
	}
	...
	function create() {
		...
		if (count) {
			...
			if (!closeall.length) {
				...
				$('<div class="msg-box-close">Ferme les toutes</div>').appendTo(container).fadeInSlide(options.fadeTime);
