	...
	return this.each(function () {
		...
		if (!$this.data('wl_Gallery')) {
			...
			$items.each(function () {
				...
				if(opts.editBtn) _append.append('<a class="edit">Ajouter</a>');
				if(opts.mailPBtn) _append.append('<a class="mailP">e-m@il</a>');
				if(opts.deleteBtn) _append.append('<a class="delete">Supprimer</a>');
			...
			if(opts.mailPBtn){
				//bind the mailP event to the button
				$this.find('a.delete').bind('click.wl_Gallery touchstart.wl_Gallery', function (event) {
					event.stopPropagation();
					event.preventDefault();
					var opts = $this.data('wl_Gallery') || opts,
